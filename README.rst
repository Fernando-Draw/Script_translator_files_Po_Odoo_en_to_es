...
```bash: pip install polib requests```

🖥️ Uso
=======
Ejecuta el script con los nombres del archivo de entrada y salida como argumentos:

```bash: python TRADUCTOR DE ARCHIVOS - PO.py archivo_entrada.po archivo_salida.po```

🔁 Ejemplo:
===========

```bash: python TRADUCTOR DE ARCHIVOS - PO.py product.pot product_es.po```
